-- Invisicam Version 2.4 (Occlusion Series)
...
function Invisicam:Update()
	...
	local currentFadeTarget = 1 - ((1 - FADE_TARGET) / currentCount)
	for hit, originalFade in pairs(SavedHits) do
		local currentFade = hit.LocalTransparencyModifier
		if currentHits[hit] then -- Fade
			if currentFade < currentFadeTarget then
				hit.LocalTransparencyModifier = math.min(currentFade + FADE_RATE, currentFadeTarget)
			elseif currentFade > currentFadeTarget then
				hit.LocalTransparencyModifier = math.max(currentFadeTarget, currentFade - FADE_RATE)
			end
		else -- Restore
